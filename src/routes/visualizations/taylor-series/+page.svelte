...
<VisualizationPage title="Taylor Series" graphID="taylor-series-graph" functions={displayFunctions}>
  {#snippet explanation()}
    <p>
      A taylor series is an infinite sum of polynomials used to approximate a function, but only
      near the point <Math latex="a" />, which is the point the series is calculated around.
    </p>
    <p>
      A taylor series works by calculating the derivatives of the original function, substituting <Math
        latex="a"
      /> for <Math latex="x" />, then integrating these derivatives to create a polynomial that will
      eventually converge to the original function.
    </p>
    <p>
      Not all taylor series will converge back to the original function, and some will only converge
      close to the point <Math latex="a" />.
    </p>
  ...
  {#snippet inputs()}
    ...
    <div>
      <input
        type="range"
        bind:value={() => iterations, (value) => setIterations(value, false)}
        min={0}
        max={15}
      />
    </div>
    ...
    <div>
      <button class="button" onclick={() => setIterations(iterations - 1)}
